...
public class CharacterIterator implements DataSetIterator {
	...
	public CharacterIterator(String path, Charset textFileEncoding, int miniBatchSize, int exampleLength,
			int numExamplesToFetch, char[] validCharacters, Random rng, boolean alwaysStartAtNewLine ) throws IOException {
		if( !new File(path).exists()) throw new IOException("Could not access file (does not exist): " + path);
		...
		List<String> lines = Files.readAllLines(new File(path).toPath(),textFileEncoding);
